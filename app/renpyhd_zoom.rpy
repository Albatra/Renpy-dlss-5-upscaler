...
#     * double-appui sur l'image : zoom d'un cran centré sur le point touché (1x → 2x → 3x → 1x, liste ZOOM_STEPS) ;
#       NB : le premier appui d'un double-appui avance le dialogue d'une ligne (comme n'importe quel appui) ;
...
#       et « ↺ » (visible seulement en zoom) qui revient à 1x.
#   PC :
#     * double-clic : cran suivant centré sur le pointeur ; Maj+Z : cran suivant centré sur l'écran ;
...
# window, menus and screens stay put. Touch (single finger, Ren'Py does not report pinch): double-tap zooms one step
# around the tapped point (1x → 2x → 3x → 1x; the first tap of a double-tap advances the dialogue like any tap), drag pans
# while zoomed (never shows an empty border), a small "x1 / x2 / x3" button (top right) cycles and "↺" resets.
# PC: double-click, Shift+Z, Ctrl + mouse wheel (the wheel alone keeps its Ren'Py role: rollback / rollforward),
# right-button (or left-button) drag pans. Deleting this file (and its .rpyc) restores the original game.
#
...
init -900 python in renpyhd_zoom:
    ...
    _st.last_pos = None
    _st.moved = False
    _st.last_tap = None  # (x, y, temps) dernier appui gauche
    _st.swallow = set()  # boutons dont le relâchement doit être ignoré (fin de glissement, 2e appui)
    ...
    def _handle(ev, x, y):
        """Observe les événements souris/tactiles sans les consommer (sauf : relâchement après glissement ou double-appui,
        Ctrl + molette). Retourne None pour laisser passer l'événement."""
        ...
        if ev.type == pygame.MOUSEMOTION:
            d = _st.down
            if d is None or not zoomed() or _in_menu():
                return None
